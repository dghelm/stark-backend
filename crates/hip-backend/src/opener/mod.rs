...
use openvm_hip_common::{copy::MemCopyD2H, d_buffer::DeviceBuffer};
...
fn commit_phase_on_gpu(
    ...
    let mut commits = vec![];
    let mut data: Vec<HipPcsData> = vec![];
...
fn answer_batch_queries_on_gpu(
    commit_phase_commits: &[HipPcsData],
    indices: &[usize],
