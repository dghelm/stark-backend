...
use openvm_hip_common::{memory_manager::MemTracker, stream::gpu_metrics_span};
...
type HB = HipBackend;
type HBChallenger = <HB as ProverBackend>::Challenger;
type HBMatrix = <HB as ProverBackend>::Matrix;
type HBVal = <HB as ProverBackend>::Val;
type HBPcsData = <HB as ProverBackend>::PcsData;
type HBCommitment = <HB as ProverBackend>::Commitment;

impl TraceCommitter<HipBackend> for HipDevice {
    #[instrument(level = "debug", skip_all)]
    fn commit(&self, traces: &[DeviceMatrix<F>]) -> (Com<SC>, HipPcsData) {
...
impl RapPartialProver<HB> for HipDevice {
    #[instrument(skip_all)]
    fn partially_prove(
        &self,
        challenger: &mut HBChallenger,
        mpk: &DeviceMultiStarkProvingKeyView<'_, HB>,
        trace_views: Vec<AirView<HBMatrix, HBVal>>,
    ) -> (
        Option<RapPhaseSeqPartialProof<SC>>,
        ProverDataAfterRapPhases<HB>,
    ) {
        ...
        let committed_pcs_data_per_phase: Vec<(Com<SC>, HipPcsData)> =
...
impl QuotientCommitter<HB> for HipDevice {
    #[instrument(skip_all)]
    fn eval_and_commit_quotient(
        &self,
        challenger: &mut HBChallenger,
        pk_views: &[&DeviceStarkProvingKey<HB>],
        public_values: &[Vec<HBVal>],
        cached_pcs_datas_per_air: &[Vec<HBPcsData>],
        common_main_pcs_data: &HBPcsData,
        prover_data_after: &ProverDataAfterRapPhases<HB>,
    ) -> (HBCommitment, HBPcsData) {
...
impl OpeningProver<HB> for HipDevice {
    #[instrument(skip_all)]
    fn open(
        &self,
        challenger: &mut HBChallenger,
        preprocessed: Vec<&HBPcsData>,
        main: Vec<HBPcsData>,
        after_phase: Vec<HBPcsData>,
        quotient_data: HBPcsData,
        quotient_degrees: &[u8],
