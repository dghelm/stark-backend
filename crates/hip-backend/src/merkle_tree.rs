//! GPU Merkle tree implementation for HIP backend.
//!
//! This module provides GPU-accelerated Merkle tree construction and querying
//! using Poseidon2 hash function.
...
impl<LDE: GpuLde> GpuMerkleTree<LDE> {
    pub fn new(leaves: Vec<LDE>, _hip_device: &HipDevice) -> Result<Self, HipError> {
    ...
    fn hash_matrices(out: &DeviceBuffer<H>, matrices: &[DeviceMatrix<F>]) -> Result<(), HipError> {
