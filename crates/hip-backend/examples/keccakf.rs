...
use openvm_hip_common::stream::current_stream_id;
...
fn main() {
    ...
    // NUM_THREADS: number of OS threads = HIP streams to use
            ...
            let worker_handle = tokio::task::spawn(async move {
                for t in start_task..end_task {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        println!("[task {t}] Starting HIP proof");
                        let engine_hip = HipBabyBearPoseidon2Engine::new(
                            FriParameters::standard_with_100_bits_conjectured_security(LOG_BLOWUP),
                        );

                        let pk_dev = engine_hip.device().transport_pk_to_device(&pk_host);
                        let hip_trace = engine_hip.device().transport_matrix_to_device(&cpu_trace);
                        let hip_ctx = ProvingContext::new(vec![(
                            air_id,
                            AirProvingContext::simple_no_pis(hip_trace),
                        )]);

                        let proof = engine_hip.prove(&pk_dev, hip_ctx);
                        engine_hip.verify(&vk, &proof).expect("verification failed");
                        println!(
