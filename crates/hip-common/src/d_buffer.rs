...
use cubecl_hip_sys::hipMemsetAsync;

use crate::{
    copy::MemCopyD2H,
    error::{check, HipError},
...
/// A struct that packs a pointer with a size in bytes to pass on HIP.
/// It holds `*const c_void` for being a universal simple type that can be read by HIP.
...
impl<T> DeviceBuffer<T> {
    ...
    pub fn with_capacity(len: usize) -> Self {
        ...
        #[cfg(feature = "touchemall")]
        {
            ...
            unsafe {
                hipMemsetAsync(raw_ptr, 0xff, size_bytes, hipStreamPerThread);
            }
        }
    ...
    pub fn fill_zero(&self) -> Result<(), HipError> {
        assert_ne!(self.len, 0, "Empty buffer");
        let size_bytes = std::mem::size_of::<T>() * self.len;
        check(unsafe { hipMemsetAsync(self.as_mut_raw_ptr(), 0, size_bytes, hipStreamPerThread) })
    }

    /// Fills a suffix of the buffer with zeros.
    /// The `start_idx` is the index in the buffer, in `T` elements.
    pub fn fill_zero_suffix(&self, start_idx: usize) -> Result<(), HipError> {
        assert!(
            start_idx < self.len,
