...
use cubecl_hip_sys::hipMemGetInfo;
...
pub(super) struct VpmmConfig {
    /// Page size override. If `None`, uses HIP's minimum granularity for the device.
...
/// Virtual memory pool implementation for HIP.
pub(super) struct VirtualMemoryPool {
    // Virtual address space roots
    pub(super) roots: Vec<hipDeviceptr_t>,

    // Map for all active pages
    active_pages: HashMap<hipDeviceptr_t, hipMemGenericAllocationHandle_t>,

    // Free regions in virtual space (sorted by address)
    free_regions: BTreeMap<hipDeviceptr_t, FreeRegion>,

    // Active allocations: (ptr, size)
    malloc_regions: HashMap<hipDeviceptr_t, usize>,

    // Unmapped regions: (ptr, size)
    unmapped_regions: BTreeMap<hipDeviceptr_t, usize>,
...
impl VirtualMemoryPool {
    pub(super) fn new(config: VpmmConfig) -> Self {
        ...
        let (root, page_size, va_size) = unsafe {
            match vpmm_check_support(device_id) {
                ...
                Err(_) => {
                    tracing::warn!("VPMM not supported, falling back to hipMallocAsync");
                    (0, usize::MAX, 0)
                }
            }
        };
        ...
        if config.initial_pages > 0 && page_size != usize::MAX {
                ...
                pool.defragment_or_create_new_pages(alloc_size, current_stream_id().unwrap())
            {
                ...
                unsafe {
                    hipMemGetInfo(&mut free_mem, &mut total_mem);
                }
                panic!(
    ...
    ///
    /// # Assumptions
    /// No HIP streams will use the malloc region starting at `ptr` after the newly recorded event
    ...
    fn free_region_insert(
        &mut self,
        mut ptr: hipDeviceptr_t,
        mut size: usize,
        stream_id: HipStreamId,
    ) -> (hipDeviceptr_t, usize) {
        ...
        let event = Arc::new(HipEvent::new().unwrap());
    ...
    /// properly handled.
    fn take_unmapped_region(&mut self, requested: usize) -> Result<hipDeviceptr_t, MemoryError> {
    ...
    fn insert_unmapped_region(&mut self, mut addr: hipDeviceptr_t, mut size: usize) {
        if size == 0 {
            return;
        }
    ...
    fn defragment_or_create_new_pages(
        ...
        requested: usize,
        stream_id: HipStreamId,
    ) -> Result<Option<hipDeviceptr_t>, MemoryError> {
        debug_assert_eq!(requested % self.page_size, 0);
        if requested == 0 {
            return Ok(None);
        }
        ...
        let mut allocated_ptr = hipDeviceptr_t::MAX;
        ...
        // - hipDeviceptr_t::MAX if no allocations occurred
        ...
        let mut to_defrag: Vec<(hipDeviceptr_t, usize)> = Vec::new();
        ...
        // Take the minimum in case allocated_ptr is hipDeviceptr_t::MAX when allocate_size = 0
    ...
    fn remap_regions(
        &mut self,
        regions: Vec<(hipDeviceptr_t, usize)>,
        dst: hipDeviceptr_t,
        stream_id: HipStreamId,
    ) -> Result<hipDeviceptr_t, MemoryError> {
        if regions.is_empty() {
            return Ok(hipDeviceptr_t::MAX);
        }
...
impl std::fmt::Debug for VirtualMemoryPool {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let mut regions: Vec<(hipDeviceptr_t, usize, String)> = Vec::new();
